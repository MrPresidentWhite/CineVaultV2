...
const HOME_DATA_CACHE_KEY = "home:data:v1";

/** TTL für den kompletten Home-Datenblock (Redis). */
const HOME_DATA_TTL = 300;
...
/**
 * Lädt Home-Daten (Collections, Filme, Serien). Ergebnis wird in Redis gecacht
 * (HOME_DATA_TTL Sekunden), damit Prisma nur bei Cache-Miss ausgeführt wird.
 */
export async function getHomeData(): Promise<HomeData> {
  return cacheGetOrSet(HOME_DATA_CACHE_KEY, HOME_DATA_TTL, async () => {
    ...
    const standaloneRaw = moviesPool.filter((m) => !m.collectionId);
    const shuffledMovies = shuffleTake(standaloneRaw, HOME_MOVIES_TAKE);
...
/**
 * Invalidiert den Home-Daten-Cache (z. B. nach Änderung an Filmen/Collections/Serien).
 * Nächster Aufruf von getHomeData() lädt frisch aus der DB.
 */
export async function invalidateHomeCache(): Promise<void> {
  await cacheDelete(HOME_DATA_CACHE_KEY);
}
